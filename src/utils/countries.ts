// Comprehensive ISO 3166-1 alpha-2 code to English country name mapping
// Source compiled from public ISO references. If a code is missing, fallback returns the input.
export const COUNTRY_NAMES: Record<string, string> = {
  ...
  LY: 'Libya', MA: 'Morocco', MC: 'Monaco', MD: 'Moldova, Republic of', ME: 'Montenegro',
  MF: 'Saint Martin (French part)', MG: 'Madagascar', MH: 'Marshall Islands', MK: 'North Macedonia', ML: 'Mali',
  ...
  TT: 'Trinidad and Tobago', TV: 'Tuvalu', TW: 'Taiwan, Province of China', TZ: 'Tanzania, United Republic of', UA: 'Ukraine',
  UG: 'Uganda', UM: 'United States Minor Outlying Islands', US: 'United States', UY: 'Uruguay', UZ: 'Uzbekistan',
  VA: 'Holy See', VC: 'Saint Vincent and the Grenadines', VE: 'Venezuela (Bolivarian Republic of)', VG: 'Virgin Islands (British)',
...
export const codeToCountryName = (codeOrName: string): string => {
  if (!codeOrName) return codeOrName;
  const code = codeOrName.toUpperCase();
  return COUNTRY_NAMES[code] || codeOrName;
};

export const countryOptionsFromCodes = (codes: string[]) => {
  return codes.map((c) => ({ value: c, label: codeToCountryName(c) }));
};
